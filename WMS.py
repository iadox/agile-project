...
def remove_item_up(items_list, old,w,c,index,m1,m2,m3,tbd):
    ...
    tbd = m2[tbd]
    if tbd in m1:
      print(f"------------------deleted from items_list-------------------")
      items_list.delete(ini)
    if tbd in m2:
      print(f"------------------deleted from warehouse list -------------------")
      w.delete(inw)
    elif tbd in m3:
      print(f"-----------------deleted from customer's list--------------------")
      c.delete(inc)
...
def merchendise(v,merch):
    ...
    label2.pack()
    items_list = Listbox(content, width=80, height=30, font=("Arial", 12))
    Warehouse_items_list = Listbox(content, width=80, height=30, font=("Arial", 12))
    Customer_items_list = Listbox(content, width=80, height=30, font=("Arial", 12))
    if v == 0:
      add_btn = HoverButton(content , text= '+' ,activebackground='#11aebf', bg='#11fad3' ,height=1,width=3, fg='black' , font=("Times New Roman", 13 ,'bold') ,  command= lambda:add_screen(items_list, Customer_items_list, Warehouse_items_list))
      add_btn.place(x=450,y=4)
      remove_btn = HoverButton(content , text= '-' ,bg='#e8db7b',activebackground='#baac41', fg='black' ,height=1,width=3, font=("Times New Roman", 13,'bold') , command= lambda:remove_item(items_list,Warehouse_items_list,Customer_items_list))
      remove_btn.place(x=400, y=4)
      upd_btn = HoverButton(content , text= 'Update' ,activebackground='#11aebf', bg='#11fad3' ,height=1,width=7, fg='black' , font=("Times New Roman", 13 ,'bold') ,  command= lambda:update_item(items_list,Warehouse_items_list,Customer_items_list,merch))
      upd_btn.place(x=270,y=4)
    if merch == 1:
      Customer_items_list.pack(padx=5, pady=5)
    elif merch == 0:
      Warehouse_items_list.pack(padx=5, pady=5)
    inv_btn = HoverButton(content , text= 'Generate Invoice' ,activebackground='#11aebf', bg='#11fad3' ,height=1,width=15, fg='black' , font=("Times New Roman", 13 ,'bold') ,  command= lambda:generate_invoice(items_list))
    inv_btn.place(x=100,y=4)
    notify_btn = HoverButton(content , text= 'Notify Customer' ,activebackground='#11aebf', bg='#11fad3' ,height=1,width=12, fg='black' , font=("Times New Roman", 13 ,'bold') ,  command= lambda:notify(items_list,Warehouse_items_list,Customer_items_list,merch))
    notify_btn.place(x=500,y=4)
...
def send_func(lis,n,em):
    ...
    body = f'''Subject: Your Item has been delivered\nDear {name}, \n\nThe items you ordered ({item_name}: Quantity:{quantity}, Total price: {quantity*price}$) have been delivered, please pick them up as soon as possible to prevent theft and make sure to set your delivery item status as Recieved.
    \nKind regards, \nThe warehouse'''
    body2 = f'''Subject: Status Update for your Item\nDear {name}, \n\nThe items you ordered ({item_name}: Quantity:{quantity}, Total price: {quantity*price}$) is currently {item_status}, Thank you for your patience, we will email you as soon as it is delivered to you.
    \nKind regards, \nThe warehouse'''
    ...
    smtpObj.ehlo()
    smtpObj.starttls()
    smtpObj.login('warehouse.system.deliveries@outlook.com', "t3mpp@ss") 
    smtpObj.sendmail('warehouse.system.deliveries@outlook.com', customer_email, body) 
    smtpObj.quit()
